...
Coordinates come from the shared preliminary candidates: `get_view_dimension_context`
(points question) or `get_structural_chain_positions`. Do not invent coordinates.
...
|---|---|---|
| Anchor | faces | An axis is allowed when the sheet already uses one. Compute it only from a qualifying pair - same `modelId`, both points having an owner-specific `supports` entry with `kind=AxisAlignedEdge` (in the legacy compact chain response use `refs[].kind`, paired with its `supportIndex`; a `TiltedEdgeCorner` alone does not qualify), non-null `partExtentAlongChain`, the two coordinates differing by exactly that extent within `partSpanMatchToleranceMm`. Otherwise faces. |
