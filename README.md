...
| `get_drawing_parts` | Модельные объекты **всего чертежа** (не одного вида): PART_POS, PART_PREFIX, ASSEMBLY_POS, PROFILE, MATERIAL, NAME. `partPrefix` — то, из чего пишется фильтр исключений; `partPrefixKnown=false` означает, что Tekla не отдала свойство, а не что префикса нет. По умолчанию ответ сжатый: записи, совпадающие во всех прочитанных полях (включая префикс и `partPrefixKnown`), объединены в одну со `count` и всеми `modelIds`; `BoltArray`, `EdgeChamfer`, `ReferenceModel`, `Connection` скрыты, но посчитаны в `hiddenByType`. `total` — объектов до фильтра, `returnedObjects` — объектов в `parts`, `returnedGroups` — записей в `parts`. `includeTypes` возвращает скрытые типы (`all` — все), `verbose=true` — полный список по одному объекту |
