...
| `get_contact_candidate_points` | Точки касания деталей вида как кандидаты для размера; каждая точка называет обе детали; `draw=true` рисует формы и точки в чертеже |
