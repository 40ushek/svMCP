# Dimensions Roadmap — archive captured 2026-09-24

Historical proposals, observations and implementation notes, preserved in full.
Some proposals below were superseded or rejected; they are not instructions for
new work. In particular, the proposal to remove stored Distance correction is
superseded: retain it and add independent rendered-line verification separately.
The only active work order is in [ROADMAP_DIMENSIONS.md](ROADMAP_DIMENSIONS.md).

## Increment: verified writes (2026-09-19; plan commands removed 2026-09-24)

Implemented: the shared create/read-back/delete protocol for dimension writes
(`writeState`). See [current contract](README.md#verified-writes-2026-09-19).

History, so this file is not misread: preview/apply commands for one steel
location chain were added on 2026-09-19 and **removed on 2026-09-24**. They
handled only one chain shape. The automatic offset they carried now lives in
`create_dimension` (omit `distance` to get the calculated offset; paper gap 8 mm,
`paperGapMm` overrides). Text below that still speaks of "plans" describes the
removed commands unless it says otherwise.
...
## Increment: a queryable view model instead of a data dump (2026-09-24)

Planned, not implemented. Decided with the user. It replaces the first idea of
one fixed short list: the model answers the question that was asked, so nobody
has to read a whole dump to find one number.

**Problem (measured on the back view `Hinten`, M.505):** the chain-positions
answer is 88 support records for only 26 distinct points, 14,013 characters
(compact) / 18,093 (verbose), about 7-9 pages of text to read before placing
one dimension. About 90% of each ~120-character record is field names, not data.

**Decision:** read Tekla once, build an in-memory model of the view from that
data, and answer small questions from the model:

- **What the model holds:** the data `get_structural_chain_positions` already
  reads: outline `extent`, the parts (model id, position, profile, main-part
  flag), the view scale, and the calculated chains with their support points.
  Building it must not read the outline a second time.
- **Where it lives:** in the persistent bridge (it stays alive for the whole
  session). In one-shot mode there is nothing to keep, so it is rebuilt per
  call; that is slower, not wrong.
- **Freshness (decided): the view is treated as unchanged while dimensions are
  being placed on it.** One snapshot per view is used for all questions and all
  dimensions of that run. Dimensions created meanwhile do not invalidate it (they
  do not change source geometry). The snapshot is rebuilt only on an explicit
  refresh, when the active drawing or the view changes, or when the model is
  missing. The existing `sourceFingerprint` is not the per-question check: it is
  built from the outline extent and calculated positions, so computing it means
  reading the geometry again, which is the work the model avoids. Stated limit:
  a change made to the model or drawing during the run is not noticed until an
  explicit refresh; there is no cheap reliable "geometry changed" signal from
  Tekla, and none is assumed.
- **How `create_dimension` finds the model:** by view id. A request carries no
  fingerprint. If the snapshot exists, use its edge and scale. If it is missing
  (first use, refresh, another drawing or view), `create_dimension` builds it
  with one outline read and then uses it.
- **First questions (small on purpose):** the points of a side sorted along its
  axis (by X for `Top`/`Bottom`, by Y for `Left`/`Right`), deduplicated, each
  with `x`, `y` (3 decimals), owning part, support kind(s) and
  `partExtentAlongChain`; the outline edge of a side; the parts of the view and
  the main part; the view scale. Any or all of the four sides can be asked.
- **Offset as one more question:** "the line for this side and these points":
  the model holds everything the offset needs (edge of the side, view scale,
  the base point of the chain), so the answer is the target line coordinate and
  the `distance` for Tekla, using the shared paper gap. `create_dimension` then
  takes the edge and the scale from the model instead of re-reading the outline
  on every call, which closes the open point left by the commit that added
  automatic offsets (the outline is read again for each dimension).
- **What is dropped from answers:** `positionIndex`/`supportIndex` (only the
  removed plan commands needed them), `sourceId`, null fields, a false `isHole`,
  a GUID per part. `GroupExtent` points (no owning part) stay available: the
  overall dimension anchors on them.
- The full raw answer (`get_structural_chain_positions`, `verbose=true`) stays
  for review.

**Name and key.** The model is called `ViewDimensionContext` (not an assembly
context: chains depend on the view, its depth window and the part filters, not
only on the assembly). One snapshot exists per **drawing + view + depth window +
filters** (excluded prefixes and materials). A different filter set is a
different snapshot, never a reused one.

**Existing pieces (reuse them, do not rebuild):** `GeometryGroup` (outline
shapes, `extent`) and `DimensionChainSet` (calculated preliminary chains) are the
content; `StructuralOutline` builds the outline; `DrawingViewContext` has the
view's parts, bounds and scale; `DimensionContext` describes dimensions that
already exist, not candidates for new ones. What is missing: a layer that builds
one context per view from these and keeps it between calls. No assembly context
exists and none is needed.

**Snapshot store.** An in-memory dictionary `key -> ViewDimensionContext` inside
the persistent bridge: get, put, and clear. It is filled on the first question or
first `create_dimension` for a view and cleared on an explicit refresh, on a
change of active drawing or view, or on bridge restart. In one-shot mode it is
empty every call, so the context is rebuilt (slower, not wrong).

**Decisions are stored separately.** "Which points are kept" is our decision, not
Tekla data; keep it in its own structure beside the context, never mixed into
the calculated chains.

**Work, in this order:**

1. `ViewDimensionContext` in `TeklaMcpServer.Api`, built from the existing pieces,
   with the questions above (side points sorted along the axis, side edge, parts
   and main part, scale, target line and offset for a side and points).
2. The snapshot store in the bridge (get, put, clear; key as above).
3. Connect `create_dimension` to it: take edge and scale from the snapshot
   instead of re-reading the outline on every call.
4. Thin question commands in the bridge and MCP that answer from the context
   (the short answers described above).
5. Route `get_structural_chain_positions` through the same context, so the two
   never build the geometry independently.
6. Update tests and the skill: drop `refs`/`supportIndex` wording, remove or
   rewrite `structured-plan.md`.

Steps 1-3 come first: they remove the repeated outline read and give the
offset a single source. Steps 4-6 follow.

**Architectural corrections to the dimension-creation path (assessed
2026-09-24).** The geometry, the calculated chains and the verified-write
protocol are sound and stay. What is wrong, and what to change:

- **Logic is in the wrong layer.** The automatic offset and the outline read
  were written directly in the bridge handler
  (`DrawingCommandHandler.Dimensions.cs`). The project rule is a thin bridge and
  all Tekla/logic in `TeklaMcpServer.Api`. Move the offset calculation and the
  context lookup into `Api`; the handler only parses arguments and calls it.
- **No shared source of view data.** Every command reads and calculates on its
  own. Replaced by `ViewDimensionContext` (above).
- **The write check does not check the line.** The verified writer forces
  `Distance` to the requested number and the read-back derives `referenceLine`
  from the extreme point, so `Verified: true` says nothing about where Tekla drew
  the line. Change: do not force `Distance` to a number whose meaning it does not
  know; after creating, read the drawn line per segment
  (`GetObjectPresentation(segmentId)`, as the probe does), compare it with the
  calculated target line, and report `matched`, `mismatch` or `not verified`
  (read failed, or a broken view where coordinates are shifted).
- **The dimension reader misreports the line.** `get_drawing_dimensions` computes
  `referenceLine` from the extreme point, not Tekla's base point. Fix it or stop
  presenting it as the drawn line.
- **Parallel mechanisms with their own numbers** (arrangement, dimension
  definitions/presets, the action plan builder, `create_dimension`). The paper
  gap is already one shared setting (8 mm); the remaining differences, for
  example the preset `DefaultDistance` (a distance for a preset, not a gap to
  the outline), must be brought under the same source or clearly separated.

These corrections come together with steps 1-3, not after them: a context that
feeds a writer which cannot check the line would only make the wrong result
faster.

**Rejected:** one fixed short list for all sides (less flexible); a side filter
as a size trick (all four sides are needed); positional tuples without field
names (deferred at the user's request); a second outline read.

**Evidence for the size (a prototype, not a result):** a deduplicated form on
the back view `Hinten` (M.505) was 1,806 characters (13% of the 14,013-character
compact answer), but without `partExtentAlongChain` and the side edges, and it
was one fixed list, not answers to each question. The real comparison is the
full proposed answer to each question against the current answer on the same
views.

**Acceptance gates:** the snapshot policy holds (test): repeated questions and
repeated dimensions on one view use one snapshot and do not read Tekla again,
creating a dimension does not drop it, and an explicit refresh, a change of
active drawing or a different view builds a new one; deduplication is by coordinate: each distinct point appears once, and
every original support and its kind stays attached to that point, so nothing the
anchor choice needs is lost (test: the set of (point, part, kind) triples before
and after is equal); all four sides can be asked (test); sort order tested;
measured characters of the full answer to each question against the current
answer on the same views, on at least three different views (a long back view,
a section, a column); the anchor check (`AxisAlignedEdge` +
`partExtentAlongChain`) is still possible from the answers; skill text that names
`refs`/`supportIndex` is updated, and the plan-command guide
`structured-plan.md`, which describes removed commands, is removed or rewritten.

**Open:** how many models to keep (one per view, or per drawing) and when to
drop them, for example when the active drawing changes; whether the default
answer should still include `GroupExtent` points; whether an answer should
carry the position `coordinate` next to the points.
