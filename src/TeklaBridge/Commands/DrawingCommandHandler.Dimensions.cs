...
internal sealed partial class DrawingCommandHandler
{
    ...
    private void WriteCombineDimensionsResult(CombineDimensionsResult result)
    {
        WriteJson(new
        {
            ...
            combined = result.Combined.Select(item => new
            {
                ...
                rollbackAttempted = item.RollbackAttempted,
                rollbackSucceeded = item.RollbackSucceeded,
                rollbackReason = item.RollbackReason,
                arrangeHandoffAttempted = item.ArrangeHandoffAttempted,
                arrangeHandoffSucceeded = item.ArrangeHandoffSucceeded,
                arrangeHandoffReason = item.ArrangeHandoffReason,
                arrangeHandoffAppliedDimensionIds = item.ArrangeHandoffAppliedDimensionIds,
                distance = item.Distance,
                reason = item.Reason,
    ...
    private static object SerializeReductionItems(System.Collections.IEnumerable? items)
    {
        ...
        return items.Cast<object>().Select(item => new
        {
            ...
            layoutRecommendedAction = GetLayoutPolicyPropertyValue(item, "RecommendedAction")?.ToString(),
            ...
            layoutCombineClassification = GetLayoutPolicyPropertyValue(item, "CombineClassification")?.ToString(),
