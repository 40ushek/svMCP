...
Используйте команду `place_control_diagonals` — она делает всё автоматически.
Ручной подход (ниже) нужен только если команда недоступна.

**Текст двух диагоналей:** обе диагонали пересекаются вблизи центра ассемблеи,
поэтому их тексты накладываются. Решение в коде: если диагонали пересекаются
(`SegmentsProperlyIntersect`), вторая размерная линия ставится на `distance * 2`
— тогда тексты расходятся. Если диагонали не пересекаются — оба `distance` одинаковые.
