...
| `get_contact_candidate_points` | Точки, где детали вида касаются друг друга, как кандидаты для размера: обе детали у каждой точки, контакт «с ребра» приходит отрезком с двумя концами, а не пятном с четырьмя углами. `draw=true` рисует результат в чертеже |
