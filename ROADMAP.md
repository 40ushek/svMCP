...
| `get_structural_outline` | То же, но за вычетом того, что исключил вызывающий: `excludePrefixes`/`excludeMaterials` (для деревянной модели — `R,S,M`: утеплитель, обшивка, крепёж). В коде исключений нет — префиксы и названия материалов у каждого завода свои и на своём языке. Ответ называет исключённые детали и то, чего не удалось прочитать |
| `get_structural_chain_positions` | Четыре предварительные цепочки позиций по структурной геометрии; позиции несут опоры, а результат — признак полноты, эхо `exclusions` и главную деталь сборки (`mainPartModelIds`, отдельно `mainPartUnresolvedModelIds` — «сборку спросить не удалось», а не «не главная») |
| `draw_structural_chain_positions` | Dev-only просмотр одной предварительной стороны цепочки линиями по структурному габариту; принимает те же исключения и сообщает их; выбор/создание размеров не выполняет |
| `get_contact_candidate_points` | Точки, где детали вида касаются друг друга, как кандидаты для размера: обе детали у каждой точки, контакт «с ребра» приходит отрезком с двумя концами, а не пятном с четырьмя углами. `draw=true` рисует результат в чертеже; `modelIds` ограничивает поиск парами внутри названного набора |
| `get_part_degrees_of_freedom` | Read-only диагностика: оси деталей без наблюдаемого подходящего контакта `FaceToFace`/`Touching`; не является политикой выбора или постановки размеров |
| `get_drawing_parts` / `get_grid_axes` | Объекты и сетка. `get_drawing_parts` отдаёт `partPrefix` с признаком `partPrefixKnown` — по этой колонке и пишется фильтр исключений |
