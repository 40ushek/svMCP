...
- Алгоритм: `get_part_geometry_in_view` для опорного объекта → пересчёт в лист-координаты → `move_view`
- Опорный объект зависит от типа чертежа:
  - **AssemblyDrawing**: главная деталь сборки — та у которой `ASSEMBLY_POS` совпадает с маркой чертежа; её СК однозначна для всех видов. ~80–100 строк, вызывается в конце `fit_views_to_sheet`
  - **GADrawing**: общая ось (`get_grid_axes`) — оси присутствуют во всех видах и однозначно идентифицируются по имени
