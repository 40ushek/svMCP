...
| `get_structural_chain_positions` | Четыре предварительные цепочки позиций по структурной геометрии; позиции несут опоры, а результат — признак полноты |
| `draw_structural_chain_positions` | Dev-only просмотр одной предварительной стороны цепочки линиями по структурному габариту; выбор/создание размеров не выполняет |
