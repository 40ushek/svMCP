...
- До транспортного рефакторинга нужен короткий baseline: contract/snapshot-тесты на `check_connection`, `get_drawing_views`, `get_drawing_parts`, `get_part_geometry_in_view`, `get_drawing_marks`, `create_dimension`.
- В baseline проверяем только инварианты: валидный JSON, ключевые поля, shape ошибок. Цель — быстро поймать тихие регрессии транспорта, а не полноценно тестировать Tekla-логику.
...
- Legacy one-shot режим по-прежнему работает без изменения CLI-команд.
- `--loop` режим стабильно обрабатывает серию последовательных вызовов без protocol desync.
